...
export interface TranscribeResponse       { transcript: TranscriptSegment[]; }
export interface SummarizeResponse        { summary: string; keyPoints: string[]; }
export interface ActionItemsResponse      { actionItems: { assignee: string; task: string; dueDate?: string; source_span?: { start: number; end: number; text: string } }[]; }
export interface SentimentResponse        { overall: string; score: number; bySpeaker: Record<string, { sentiment: string; score: number }>; }
export interface IdentifySpeakersResponse { speakerMap: Record<string, { name: string; email: string }>; }
export interface InsightsResponse {
  insights: { speaker: string; name: string; talkTimeSeconds: number; talkTimePercent: number; skills: string[]; suggestions: string[] }[];
}
export interface MomResponse              { agenda: string[]; discussionPoints: { speaker: string; point: string }[]; summary: string; }
export interface DecisionsResponse        { decisions: { decision: string; madeBy: string; rationale?: string }[]; }
export interface DeadlinesResponse        { deadlines: { description: string; assignee: string; deadline: string; rawText: string }[]; }
export interface SkillMatchResponse       { requiredSkills: string[]; matchedUserId: string | null; confidence: number; }
export interface EffectivenessScoreResponse {
  score: number;
  breakdown: { decisionsScore: number; keyPointsCoverage: number; participationBalance: number };
  suggestions: string[];
}
...
export const transcribeAudio = async (fileBuffer: Buffer, mimetype: string): Promise<TranscribeResponse> => {
  if (useMocks) return transcribeFixture;
  try {
    return await withAi1(async () => {
      const form = new FormData();
      form.append('file', fileBuffer, { contentType: mimetype, filename: 'audio' });
      const { data } = await ai1.post<TranscribeResponse>('/internal/ai/transcribe', form, {
...
export const summarizeMeeting = async (transcript: TranscriptSegment[], meetingTitle: string): Promise<SummarizeResponse> => {
  if (useMocks) return summarizeFixture;
  return ai1Post('/internal/ai/summarize', { transcript, meetingTitle });
};

export const extractActionItems = async (transcript: TranscriptSegment[]): Promise<ActionItemsResponse> => {
  if (useMocks) return actionItemsFixture;
  return ai1Post('/internal/ai/action-items', { transcript });
};

export const generateMom = async (transcript: TranscriptSegment[], meetingTitle: string): Promise<MomResponse> => {
  if (useMocks) return momFixture;
  return ai1Post('/internal/ai/mom', { transcript, meetingTitle });
};

export const extractDecisions = async (transcript: TranscriptSegment[]): Promise<DecisionsResponse> => {
  if (useMocks) return decisionsFixture;
  return ai1Post('/internal/ai/decisions', { transcript });
};

export const extractDeadlines = async (transcript: TranscriptSegment[]): Promise<DeadlinesResponse> => {
  if (useMocks) return deadlinesFixture;
  return ai1Post('/internal/ai/deadlines', { transcript });
};

export const analyzeSentiment = async (transcript: TranscriptSegment[]): Promise<SentimentResponse> => {
...
export const identifySpeakers = async (
  ...
  return ai2Post('/internal/ai/identify-speakers', { transcript, participants });
};
...
export const scoreEffectiveness = async (input: {
  ...
  keyPoints: string[];
  talkTime: { speaker: string; talkTimeSeconds: number; talkTimePercent: number }[];
