...
const diffHunkSchema = new Schema<DiffHunkDoc>(
  { op: { type: String, enum: ['unchanged', 'removed', 'added'], required: true }, value: { type: String, required: true } },
  { _id: false },
);
...
export const ReviewVersion = model<IReviewVersion>('ReviewVersion', reviewVersionSchema);
