...
router.get('/', requireAuth, validateQuery(querySchema), async (req: AuthRequest, res: Response) => {
  ...
  if (meeting.processingStatus !== 'completed') {
    res.status(409).json({ message: 'Tasks not available yet', processingStatus: meeting.processingStatus });
    return;
  }
