...
router.get(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { format } = res.locals['parsedQuery'] as z.infer<typeof exportQuerySchema>;
      ...
      await getExportQueue().add('generate', { jobId: exportJob.id, meetingId, format });
...
router.get('/:jobId', requireAuth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const meetingId = req.params['id'] as string;
    ...
    if (req.user!.role === 'employee' && exportJob.requestedBy.toString() !== req.user!.sub) {
      res.status(403).json({ code: 'ACCESS_DENIED', message: 'Access denied' });
      return;
    }
    ...
    if (exportJob.status !== 'done') {
      res.json({
        ...
        ...(exportJob.errorMessage
          ? { code: 'EXPORT_FAILED', error: exportJob.errorMessage }
          : {}),
      });
      return;
    }
    ...
    if (!fs.existsSync(filePath)) {
      res.status(410).json({ code: 'FILE_EXPIRED', message: 'Export file no longer available' });
      return;
    }
    ...
    res.setHeader('Content-Disposition', `attachment; filename="meeting-${meetingId}.${exportJob.format}"`);
