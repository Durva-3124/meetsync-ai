  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!task) {
        res
          .status(404)
          .json({ code: 'TASK_NOT_FOUND', message: 'Task not found' });
        return;
      }
      ...
      const meeting = await Meeting.findById(task.meetingId).select(
        'title createdBy participants'
      );
      if (!meeting) {
        res
          .status(404)
      ...
      const recipient = await User.findById(task.matchedUserId).select(
        'name email'
      );
      if (!recipient) {
        res.status(422).json({
